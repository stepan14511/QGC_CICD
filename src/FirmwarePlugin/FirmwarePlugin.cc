...
void FirmwarePlugin::landingStationSetMode(Vehicle* vehicle, uint8_t landingStationMode)
{
    sendLandingStationCmd(vehicle, landingStationMode);
}
...
void FirmwarePlugin::landingStationSetElevatorMode(Vehicle* vehicle, uint8_t elevatorMode)
{
...
const QVariantList& FirmwarePlugin::toolIndicators(const Vehicle*)
{
    ...
    if(_toolIndicatorList.size() == 0) {
        _toolIndicatorList = QVariantList({
            ...
            QVariant::fromValue(QUrl::fromUserInput("qrc:/toolbar/GripperIndicator.qml")),
            QVariant::fromValue(QUrl::fromUserInput("qrc:/toolbar/LandingStationIndicator.qml")),
...
void FirmwarePlugin::sendLandingStationCmd(Vehicle* vehicle, uint8_t cmd)
{
    WeakLinkInterfacePtr weakLink = vehicle->vehicleLinkManager()->primaryLink();
    if (!weakLink.expired()) {
        MAVLinkProtocol*        mavlinkProtocol = qgcApp()->toolbox()->mavlinkProtocol();
        mavlink_landing_station_cmd_t msg_cmd   = {};
        SharedLinkInterfacePtr  sharedLink      = weakLink.lock();
        ...
        mavlink_message_t message;
        mavlink_msg_landing_station_cmd_encode_chan(static_cast<uint8_t>(mavlinkProtocol->getSystemId()),
                                                    static_cast<uint8_t>(mavlinkProtocol->getComponentId()),
                                                    sharedLink->mavlinkChannel(),
                                                    &message,
                                                    &msg_cmd);
...
void FirmwarePlugin::sendLandingStationElevatorCmd(Vehicle* vehicle, uint8_t cmd)
{
    WeakLinkInterfacePtr weakLink = vehicle->vehicleLinkManager()->primaryLink();
    if (!weakLink.expired()) {
        MAVLinkProtocol*        mavlinkProtocol = qgcApp()->toolbox()->mavlinkProtocol();
        mavlink_landing_station_elevator_cmd_t msg_cmd   = {};
        SharedLinkInterfacePtr  sharedLink      = weakLink.lock();
...
void FirmwarePlugin::sendLandingStationChargingCmd(Vehicle* vehicle, uint8_t cmd)
{
    WeakLinkInterfacePtr weakLink = vehicle->vehicleLinkManager()->primaryLink();
    if (!weakLink.expired()) {
        MAVLinkProtocol*        mavlinkProtocol = qgcApp()->toolbox()->mavlinkProtocol();
        mavlink_landing_station_charging_cmd_t msg_cmd   = {};
        SharedLinkInterfacePtr  sharedLink      = weakLink.lock();
        ...
        mavlink_message_t message;
        mavlink_msg_landing_station_charging_cmd_encode_chan(static_cast<uint8_t>(mavlinkProtocol->getSystemId()),
                                                       static_cast<uint8_t>(mavlinkProtocol->getComponentId()),
                                                       sharedLink->mavlinkChannel(),
                                                       &message,
                                                       &msg_cmd);
